  ...
  if $manage_repo {
    case $facts['os']['family'] {
      ...
      'Debian': {
        ...
        # apt-key is gone as of Ubuntu 26.04, so the key is deployed as a keyring
        # in /etc/apt/keyrings and referenced via signed-by. zabbix-official-repo.key
        # is ASCII armored and holds every key Zabbix signs its repositories with,
        # hence the .asc suffix and the single keyring.
        apt::source { 'zabbix':
